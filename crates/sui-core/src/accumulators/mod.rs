...
impl AccumulatorSettlementTxBuilder {
    ...
    /// Returns a unified map of balance changes for all accounts.
    /// The balance change for each account is merged from the merge and split operations.
    pub fn collect_balance_changes(&self) -> BTreeMap<ObjectID, i128> {
        self.updates
            .iter()
