...
impl AccountState {
    ...
    #[instrument(level = "debug", skip_all, fields(cur_accumulator_version =? cur_accumulator_version.value(), account_id = ?self.object_id, tx_digest = ?pending_withdraw.tx_digest, ?has_blocking_reservations))]
    fn try_reserve(
        &mut self,
    ...
    fn debug_check_account_state_invariants(
        ...
        let actual_balance =
            balance_read.get_account_balance(&self.object_id, last_settled_version);
