...
pub(crate) async fn upgrade_package(
    ...
    let compiled_package =
        compile_package(read_api, root_pkg, build_config.clone(), package_path).await?;
...
pub(crate) async fn compile_package(
    ...
    check_version_compatibility(read_api, &compiled_package).await?;

    pkg_tree_shake(read_api, &mut compiled_package).await?;
...
pub(crate) async fn pkg_tree_shake(
    ...
    let root_modules: Vec<_> = compiled_package
        .package
        .root_modules()
        .map(|x| x.unit.clone())
        .collect();
...
async fn publish_command(
    ...
    let PublishArgs {
        ...
        verify_deps: _,
        with_unpublished_dependencies: _,
        payment,
        gas_data,
        processing,
    } = args;
    ...
    let compiled_package =
        compile_package(read_api, &root_package, build_config.clone(), &package_path).await;
