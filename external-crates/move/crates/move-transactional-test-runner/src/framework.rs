...
use move_vm_runtime::dev_utils::vm_arguments::ContextualizedValues;
use std::{
...
pub trait MoveTestAdapter<'a>: Sized + Send {
    ...
    ) -> Result<(Option<String>, ContextualizedValues)>;
...
fn display_return_values(return_values: ContextualizedValues) -> Option<String> {
    let ContextualizedValues {
        context,
        heap_refs,
    ...
    if !heap_refs.is_empty() {
        let values = heap_refs
            .iter()
            .map(|(idx, heap_id)| {
                let value = context.borrow_mut().take_loc(*heap_id).unwrap();
                (idx, value)
            })
            .collect::<Vec<_>>();
        let printed = values
            ...
            .collect::<Vec<_>>()
            .join(", ");
        output.push(format!("reference inputs after call: {}", printed))
    };
