...
fn call_vm_function(
    ...
    let ContextualizedValues {
        ...
        heap_refs,
        values,
    } = allocate_args_for_call(vm_instance, module, function, &type_args, args)?;

    println!("Doing call");
    let result = vm_instance
        .execute_function_bypass_visibility(module, function, type_args, values, gas_meter, None);
    println!("Done calling");
    result.map(|return_values| ContextualizedValues {
        context,
        heap_refs,
