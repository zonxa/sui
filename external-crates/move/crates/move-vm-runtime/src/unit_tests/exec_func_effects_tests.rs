...
fn mutref_output_success() {
    ...
    assert!(ret_values.heap_refs.len() == 1);
    let id = *ret_values.heap_refs.iter().next().unwrap().1;
    let v = ret_values.context.borrow_mut().take_loc(id).unwrap();
...
fn run(module: &ModuleCode, fun_name: &str, arg_val0: MoveValue) -> VMResult<ContextualizedValues> {
    ...
    session
        ...
        .map(|return_values| ContextualizedValues {
            context,
            heap_refs,
