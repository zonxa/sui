...
use crate::{
    dev_utils::{
        ...
        vm_arguments::ContextualizedValues,
...
fn run_with_module(
...
) -> (VMResult<ContextualizedValues>, u128) {
    ...
    let res = type_args.and_then(|type_args| {
        vm_session.execute_entry_function(&module_id, entry_name.as_ref(), type_args, vec![], gas)
    });

    let time = now.elapsed().as_millis();
    (res.map(ContextualizedValues::no_refs), time)
}
