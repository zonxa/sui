...
impl Local<'_> {
    fn resolved_location(&self) -> Result<BaseHeapId, ExecutionError> {
        self.0
            .locations
            .get(&self.1)
            .copied()
    ...
    pub fn is_invalid(&self) -> Result<bool, ExecutionError> {
        self.0
            .heap
            .is_invalid(self.resolved_location()?)
    ...
    pub fn store(&mut self, value: Value) -> Result<(), ExecutionError> {
        let val: values::Reference = self
            .0
            .heap
            .borrow_loc(self.resolved_location()?)
    ...
    pub fn move_(&mut self) -> Result<Value, ExecutionError> {
        ...
        self.0
            .heap
            .take_loc(self.resolved_location()?)
            .map_err(iv("move loc"))
            .map(Value)
    }
